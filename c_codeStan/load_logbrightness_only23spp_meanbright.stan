data {
  ...
  int<lower=0> N_meanbright;            // Number of observations for mean brightness
  ...
  array[N_meanbright] int<lower=1, upper=N_sp_nem> species_kmean; // New species id for mean brightness dataset
  
  vector[N_sp_nem] mean_brightness;
  vector[N_sp_nem] sd_brightness;
...
parameters {
  real intercept;
  real slope_bright; //real<lower=0> slope_bright;
...
model {
  ...
  mu_bright ~ normal(mean_brightness[species_kmean], sd_brightness[species_kmean]);
