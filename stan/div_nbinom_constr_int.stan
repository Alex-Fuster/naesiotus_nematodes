...
parameters {
  ...
  vector[N_island] slope_bright_div_island;  // Slope of brightness-diversity relationship per island
...
model {
  ...
  // Priors for island-specific slopes
  slope_bright_div_island ~ normal(0, 1); // Prior for the slope for each island
  ...
  slope_bright_div_island[island_index_spp] .* true_ln_bright +  // Interaction term to allow varying slope by island
...
generated quantities {
  ...
  for (n in 1:N_div) {
    ...
    //  slope_bright_div_island[island_index_spp] .* true_ln_bright +
