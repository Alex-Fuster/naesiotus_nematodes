...
parameters {
  ...
  real slope_bright_load;  // General effect of brightness on load across all islands
  vector[N_island] slope_bright_load_island;  // Island-specific slopes for brightness on load
  real<lower=0> sd_bright_load_island;  // Standard deviation for island-specific slopes
...
model {
  ...
  // Prior for island-specific slopes for brightness
  slope_bright_load_island ~ normal(0, sd_bright_load_island);  // Island-specific variation around the general slope
  
  // Prior for standard deviation of island-specific slopes for brightness
  sd_bright_load_island ~ exponential(1);
  ...
  slope_arid_bright * arid_prob +
    slope_age_bright * island_age_true[island_index_spp] + // Use island_age_true at species level
  slope_area_bright * log_island_area[island_index_spp];  // Use species-level log_island_area
  ...
  island_effect_load[island_index_spp] +
   (slope_bright_load + slope_bright_load_island[island_index_spp]) .* true_ln_bright +  // Element-wise multiplication  // Combined general and island-specific brightness effect
  slope_arbor_load * arboreal_prob +
  slope_arid_load * arid_prob +
  slope_age_load * island_age_true[island_index_spp] +  // Use island_age_true at species level
  slope_area_load * log_island_area[island_index_spp];  // Use species-level log_island_area
...
generated quantities {
  ...
  vector[N_island] total_slope_bright_load_island;
  
  for (i in 1:N_island) {
    total_slope_bright_load_island[i] = slope_bright_load + slope_bright_load_island[i];
  }
