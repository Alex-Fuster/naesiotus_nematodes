...
Predicted values have super high values - I need to solve this in the generated quantities block, probably back transforming


## Negative binomial model with informed priors on brightness
...
# Data code including phylogenetic covariance matrix
constr_bright_samp <- all_spp_load_constr_bright$sample(
  data = list(
  N_spp = nrow(sp_code_island_df),  # Number of species (47)
  N_island = max(sp_code_island_df$island_code),
  sp_id = sp_code_island_df$species_code,
  ...
  chains = 10, parallel_chains = 10, refresh = 0,
iter_sampling = 500,
output_dir = here::here("output/models/missing_data_load_nbinom_constr_bright")
)
...
```{r}
# Extract the posterior samples
posterior_samples <- constr_bright_samp$draws()
...
```{r}
#| class-output: stan
all_spp_load_constr_brightload <- cmdstan_model(here::here("stan/all_spp_load_nbinom_constr_brightload.stan"),
                                          pedantic = TRUE)
...
# Data code including phylogenetic covariance matrix
constr_brightload_samp <- all_spp_load_constr_brightload$sample(
  data = list(
  N_spp = nrow(sp_code_island_df),  # Number of species (47)
  N_island = max(sp_code_island_df$island_code),
  sp_id = sp_code_island_df$species_code,
  ...
  chains = 10, parallel_chains = 10, refresh = 0,
iter_sampling = 500,
output_dir = here::here("output/models/missing_data_load_nbinom_constr_brightload")
)
...
```{r}
# read in the model from above
constr_brightload_samp <- as_cmdstan_fit(dir(here::here("output/models/missing_data_load_nbinom_constr_brightload"), full.names = TRUE))
```
...
predicted_load <- constr_brightload_samp$draws("predicted_nematode_load")
...
#ggsave(here::here("output/figures/predictions/pred_load_constr_brightload.png"), height = 4, width = 5)
...
```{r}
bright_rvars <- constr_brightload_samp |> 
  tidybayes::gather_rvars(true_ln_bright[species_code]) |> 
  mutate(brightness = exp(.value),
         species_code = forcats::fct_reorder(as.character(species_code), .value))
...
bright_rvars <- bright_rvars %>%
  mutate(species_name = fct_reorder(species_name, brightness))
...
#ggsave(here::here("output/figures/predictions/pred_bright_constr_brightload.png"), height = 7, width = 7)
```
