...
#saveRDS(sp_code_island_df, here::here("output/temporal_output/sp_island_id.rds"))
```

## Nematode diversity - faith diversity from sequencing analyses
...
faith_pd_table <- faith_pd_table %>%
  left_join(species_names_df |> select(species, species_code,species_name),
            by = "species") |> 
  left_join(sp_code_island_df |> select(island, island_code, species_name),
            by = "species_name")
```
...
faith_pd_table <- faith_pd_table %>%
  mutate(species_name = factor(species_name, levels = species_name[order(island)]))

ggplot(faith_pd_table, aes(x = species_name, y = faith_pd, fill = island)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Faith's Phylogenetic Diversity", x = "Species", y = "Faith PD")
```
...
```{r}
df_mismatch <- read_csv(here::here("data/processed/mismatch_df.csv"))
df_mismatch <- df_mismatch %>% 
  select(location, spp, mismatch) %>% 
  dplyr::rename(id = location,
         species = spp) %>%
  left_join(species_names_df |> select(species, species_code), by = "species")
...
island_df <- sp_code_island_df |>
  ...
  dplyr::summarise(
    ...
    island_area = dplyr::first(island_area)  # Assuming all rows for the same island have the same area
  ) |>
...
```{r}
# write.csv(island_df, here::here("data/processed/df_for_models/island_df.csv"))
# write.csv(sp_code_island_df, here::here("data/processed/df_for_models/sp_code_island_df.csv"))
# write.csv(df_load, here::here("data/processed/df_for_models/df_load.csv"))
# write.csv(faith_pd_table, here::here("data/processed/df_for_models/faith_pd_table.csv"))
# write.csv(df_bright_noNA, here::here("data/processed/df_for_models/df_bright_noNA.csv"))
# write.csv(df_veg, here::here("data/processed/df_for_models/df_veg.csv"))
# write.csv(df_hab, here::here("data/processed/df_for_models/df_hab.csv"))
# write.csv(df_mismatch, here::here("data/processed/df_for_models/df_mismatch.csv"))
```


```{r}
island_df <- read.csv(here::here("data/processed/df_for_models/island_df.csv"))
sp_code_island_df <- read.csv(here::here("data/processed/df_for_models/sp_code_island_df.csv"))
df_load <- read.csv(here::here("data/processed/df_for_models/df_load.csv"))
faith_pd_table <- read.csv(here::here("data/processed/df_for_models/faith_pd_table.csv"))
df_bright_noNA <- read.csv(here::here("data/processed/df_for_models/df_bright_noNA.csv"))
df_veg <- read.csv(here::here("data/processed/df_for_models/df_veg.csv"))
df_hab <- read.csv(here::here("data/processed/df_for_models/df_hab.csv"))
df_mismatch <- read.csv(here::here("data/processed/df_for_models/df_mismatch.csv"))
```
...
# Define a named vector to map parameter names to variable names (removing ?? notation)
parameter_labels <- c(
  "slope_bright_load" = "Brightness",
  "slope_arbor_bright" = "Arboreous",
  "slope_arid_bright" = "Arid",
  "slope_area_bright" = "Island Area",
  "slope_age_bright" = "Island Age",
  
  "slope_arbor_load" = "Arboreous",
  "slope_arid_load" = "Arid",
  "slope_area_load" = "Island Area",
  "slope_age_load" = "Island Age"
)
...
# Define number of samples to match the posterior size
n_prior_samples <- nrow(posterior_long)
...
custom_order <- c("Brightness", "Arboreous", "Arid", "Island Area", "Island Age")
...
# **Split dataset into two separate ones**
# One for the effects on brightness
brightness_data <- combined_data %>%
  filter(parameter %in% c("slope_arbor_bright", "slope_arid_bright", 
                          "slope_area_bright", "slope_age_bright")) %>%
  mutate(parameter = factor(parameter, levels = names(parameter_labels), 
                            labels = unname(parameter_labels))) %>%
  mutate(parameter = factor(parameter, levels = rev(custom_order)))  # Set custom order

# One for the effects on nematode load
load_data <- combined_data %>%
  filter(parameter %in% c("slope_bright_load", "slope_arbor_load", "slope_arid_load", 
                          "slope_area_load", "slope_age_load")) %>%
  mutate(parameter = factor(parameter, levels = names(parameter_labels), 
                            labels = unname(parameter_labels))) %>%
  mutate(parameter = factor(parameter, levels = rev(custom_order)))  # Set custom order

# Ridge plot for **effects on Brightness**
plot_brightness <- ggplot(brightness_data, aes(x = value, y = parameter, fill = type)) +
  geom_density_ridges(
    alpha = 0.5, scale = 1.2, rel_min_height = 0.01
  ) +
  scale_fill_manual(values = c("Posterior" = "lightcoral", "Prior" = "grey80")) +
  theme_minimal() +
  labs(
    title = "Effects on shell brightness",
    x = "Coefficient Estimate (??)",
    y = ""  # Simplified y-axis labels
  ) +
...
# Ridge plot for **effects on Load**
plot_load <- ggplot(load_data, aes(x = value, y = parameter, fill = type)) +
  geom_density_ridges(
    alpha = 0.5, scale = 1.2, rel_min_height = 0.01
  ) +
  scale_fill_manual(values = c("Posterior" = "lightcoral", "Prior" = "grey80")) +
  theme_minimal() +
  labs(
    title = "Effects on encapsulations count",
    x = "Coefficient Estimate (??)",
    y = ""  # Simplified y-axis labels
  ) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
...
```{r}
posterior_summary <- summarise_draws(unconstr_brightload_samp)

# Extract R-hat values
rhat_values <- posterior_summary$rhat

ggsave(here::here("output/figures/model_checks/load_model_rhat.png"),
...
# Use a grayscale color scheme for all bayesplot functions
...
```{r}
ggsave(here::here("output/figures/model_checks/load_model_nef.png"),
       neff_ratio(unconstr_brightload_samp) |> 
         mcmc_neff_hist() + 
         scale_fill_manual(values = c("gray30", "gray60", "gray90")),
       height = 5, width = 5)
...
```{r}
ggsave(here::here("output/figures/model_checks/load_model_autocorrelation_plots.png"),
       mcmc_acf(selected_params) +
...
```{r}
ggsave(here::here("output/figures/model_checks/load_model_hist_trace_plots.png"),
       wrap_plots(
         mcmc_hist(selected_params, facet_args = list(nrow = 3)) + 
           scale_fill_manual(values = c("gray30", "gray60", "gray90")),
         mcmc_trace(selected_params, facet_args = list(nrow = 3)) +
           scale_color_manual(values = c("gray30", "gray60", "gray90")),
         widths = c(1, 1.5)
       ),
       height = 7, width = 12)
...
```{r}
# Extract the draws for 'log_avg_predicted_load'
predicted_load_draws <- unconstr_brightload_samp$draws(variables = "log_avg_predicted_load")

# Convert to a data frame
predicted_load_data <- as.data.frame(predicted_load_draws)

# Check the unique draw IDs
unique_draw_ids <- unique(predicted_load_tidy$draw_id)
length(unique_draw_ids)  # This should give you an idea of the structure


# Extract the species number from the draw_id
predicted_load_tidy <- predicted_load_tidy %>%
  mutate(species_code = as.numeric(sub("\\..*", "", draw_id)))  # Extract species from draw_id


# Calculate the mean predicted load for each species
predicted_load_summary <- predicted_load_tidy %>%
  group_by(species_code) %>%
  summarize(mean_log_avg_predicted_load = mean(log_avg_predicted_load))

# Transform back to raw scale
predicted_load_summary <- predicted_load_summary %>%
  mutate(mean_avg_predicted_load = exp(mean_log_avg_predicted_load))

# Merge with species metadata
df_pred <- sp_code_island_df %>%
  left_join(predicted_load_summary, by = "species_code")

# Plot the overall distribution of predicted nematode load for all species
ggplot(df_pred, aes(x = mean_avg_predicted_load)) +
  geom_density(fill = "blue", alpha = 0.5) +
  labs(title = "Distribution of Predicted Nematode Load (All Species)",
       x = "Predicted Load", y = "Density") +
  theme_classic()
...
```{r}
#| class-output: stan
div_nbinom_unconstr <- cmdstan_model(here::here("stan/div_nbinom_unconstr_2_trueage_islandindex_v2.stan"),
                                          pedantic = TRUE)
```

```{r}
# Use island-level data in Stan
unconstr_brightdiv_samp <- div_nbinom_unconstr$sample(
  data = list(
    N_spp = nrow(sp_code_island_df),  # Number of species (47)
    N_island = nrow(island_df),  # Number of unique islands
    sp_id = sp_code_island_df$species_code,
    ...
    sp_index_bright = df_bright_noNA$species_code,
    # Nematode load data
    N_div = nrow(faith_pd_table),  # Total observations for nematode load
    faith_pd = faith_pd_table$faith_pd,
    sp_index_div = faith_pd_table$species_code,  # Use species_code from df_load
    island_index_div = faith_pd_table$new_island_index  # Use island_code from df_load
  ),
  chains = 6, #10, 
  parallel_chains = 6, #10, 
  refresh = 0, 
  iter_warmup = 3000, #2000,  #  Increase warmup iterations
  iter_sampling = 6000, #4000,  #  Increase total samples
  adapt_delta = 0.999, #0.99,  #  Reduce divergent transitions
  max_treedepth = 18, #15,  #  Allow longer trajectories
  output_dir = here::here("output/models/missing_data_div_nbinom_unconstr_2_trueage_islandindex_v2/model_output")
)
...
```{r}
# read in the model from above
unconstr_brightdiv_samp <- as_cmdstan_fit(dir(here::here("output/models/missing_data_div_nbinom_unconstr_2_trueage_islandindex_v2/model_output"), full.names = TRUE))
```
...
```{r}
#get_variables(unconstr_brightload_samp)
unconstr_brightdiv_samp$diagnostic_summary()
...
```{r}
# Extract posterior samples
posterior_samples <- unconstr_brightdiv_samp$draws()
posterior_df <- as_draws_df(posterior_samples)
```
...
# Convert posterior samples into a dataframe
posterior_samples_div <- as_draws_df(unconstr_brightdiv_samp)

# Define parameter names for the diversity model
parameter_labels_diversity <- c(
  "slope_bright_div" = "Brightness",
  "slope_arbor_bright" = "Arboreous",
  "slope_arid_bright" = "Arid",
  "slope_area_bright" = "Island Area",
  "slope_age_bright" = "Island Age",
  
  "slope_arbor_div" = "Arboreous",
  "slope_arid_div" = "Arid",
  "slope_area_div" = "Island Area",
  "slope_age_div" = "Island Age"
)
...
# Convert posterior samples to long format
posterior_div_long <- posterior_samples_div %>%
  select(starts_with("slope_arbor"), starts_with("slope_arid"),
         starts_with("slope_age"), starts_with("slope_area"), 
         slope_bright_div) %>%
  pivot_longer(everything(), names_to = "parameter", values_to = "value") %>%
...
# Define number of samples to match the posterior size
n_prior_samples <- nrow(posterior_div_long)
...
# Convert prior samples to long format
priors_div_long <- priors_div %>%
  pivot_longer(everything(), names_to = "parameter", values_to = "value") %>%
  mutate(type = "Prior")

# Combine prior and posterior data
combined_data_div <- bind_rows(priors_div_long, posterior_div_long)

# **Split dataset into two separate ones**
# One for the effects on brightness
brightness_data_div <- combined_data_div %>%
  filter(parameter %in% c("slope_arbor_bright", "slope_arid_bright", 
                          "slope_area_bright", "slope_age_bright")) %>%
  mutate(parameter = factor(parameter, levels = names(parameter_labels_diversity), 
                            labels = unname(parameter_labels_diversity))) %>%
  mutate(parameter = factor(parameter, levels = rev(custom_order)))  # Set custom order

# One for the effects on nematode diversity
diversity_data <- combined_data_div %>%
  filter(parameter %in% c("slope_bright_div", "slope_arbor_div", "slope_arid_div", 
                          "slope_area_div", "slope_age_div")) %>%
  mutate(parameter = factor(parameter, levels = names(parameter_labels_diversity), 
                            labels = unname(parameter_labels_diversity))) %>%
  mutate(parameter = factor(parameter, levels = rev(custom_order)))  # Set custom order

# Ridge plot for **effects on Brightness**
plot_brightness_div <- ggplot(brightness_data_div, aes(x = value, y = parameter, fill = type)) +
  geom_density_ridges(
    alpha = 0.5, scale = 1.2, rel_min_height = 0.01
  ) +
  scale_fill_manual(values = c("Posterior" = "lightcoral", "Prior" = "grey80")) +
  theme_minimal() +
  labs(
    title = "Posterior Distributions for Effects on Brightness",
    x = "Coefficient Estimate (??)",
    y = ""  # Simplified y-axis labels
  ) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
...
# Ridge plot for **effects on Diversity**
plot_diversity <- ggplot(diversity_data, aes(x = value, y = parameter, fill = type)) +
  geom_density_ridges(
    alpha = 0.5, scale = 1.2, rel_min_height = 0.01
  ) +
  scale_fill_manual(values = c("Posterior" = "lightcoral", "Prior" = "grey80")) +
  theme_minimal() +
  labs(
    title = "Effects on nematode diversity",
    x = "Coefficient Estimate (??)",
    y = ""  # Simplified y-axis labels
  ) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
...
```{r}
posterior_summary <- summarise_draws(unconstr_brightdiv_samp)

# Extract R-hat values
rhat_values <- posterior_summary$rhat

ggsave(here::here("output/figures/model_checks/div_model_rhat.png"),
       mcmc_rhat_hist(rhat_values),
       height = 5,
       width = 5)
...
```{r}
ggsave(here::here("output/figures/model_checks/div_model_nef.png"),
       neff_ratio(unconstr_brightdiv_samp) |> 
         mcmc_neff_hist() + 
         scale_fill_manual(values = c("gray30", "gray60", "gray90")),
       height = 5, width = 5)
...
```{r}
ggsave(here::here("output/figures/model_checks/div_model_autocorrelation_plots.png"),
       mcmc_acf(selected_params) +
         scale_color_manual(values = c("gray30", "gray60", "gray90")))

```
```{r}
ggsave(here::here("output/figures/model_checks/div_model_hist_trace_plots.png"),
       wrap_plots(
         mcmc_hist(selected_params, facet_args = list(nrow = 3)) + 
           scale_fill_manual(values = c("gray30", "gray60", "gray90")),
         mcmc_trace(selected_params, facet_args = list(nrow = 3)) +
...
# Gather posterior predictions for arboreal habitat probability
hab_rvars <- unconstr_brightdiv_samp |> 
  tidybayes::gather_rvars(arboreal_prob[sp_index_hab]) |> 
  mutate(species_code = forcats::fct_reorder(as.character(sp_index_hab), .value))
...
#ggsave(here::here("output/models/missing_data_div_nbinom_unconstr_2_trueage_islandindex/p_pred_hab.png"), height = 6, width = 9)
...
# Gather posterior predictions for arid vegetation probability
veg_rvars <- unconstr_brightdiv_samp |> 
  tidybayes::gather_rvars(arid_prob[sp_index_veg]) |> 
  mutate(species_code = forcats::fct_reorder(as.character(sp_index_veg), .value))
...
#ggsave(here::here("output/models/missing_data_div_nbinom_unconstr_2_trueage_islandindex/p_pred_veg.png"), height = 6, width = 9)
...
```{r}
bright_rvars <- unconstr_brightdiv_samp |> 
  tidybayes::gather_rvars(true_ln_bright[species_code]) |> 
  mutate(brightness = exp(.value),
         species_code = forcats::fct_reorder(as.character(species_code), .value))
...
#ggsave(here::here("output/models/missing_data_div_nbinom_unconstr_2_trueage_islandindex/p_pred_bright.png"), height = 6, width = 9)
```
...
```{r}
# Gather predicted Faith PD values for each observation (N_div)
div_rvars <- unconstr_brightdiv_samp |>
  tidybayes::gather_rvars(predicted_faith_pd[n])

# Combine with observed data
div_rvars <- div_rvars |>
  mutate(
    species_code = faith_pd_table$species_code[n],
    observed_faith_pd = faith_pd_table$faith_pd[n]
  )


# Separate `.value` into mean and error columns
div_rvars_clean <- div_rvars %>%
  mutate(.value = as.character(.value)) %>%  # Ensure `.value` is treated as a character
  separate(.value, into = c("mean_predicted", "error_predicted"), sep = " ± ", convert = TRUE)

# Add credible intervals using the mean ± 1.96 * SD (approximation for 95% CI)
div_rvars_clean <- div_rvars_clean %>%
  mutate(
    lower_ci = mean_predicted - 1.96 * error_predicted,
    upper_ci = mean_predicted + 1.96 * error_predicted
  )

# Plot observed vs predicted Faith PD with credible intervals
ggplot(div_rvars_clean, aes(x = observed_faith_pd, y = mean_predicted)) +
  geom_point(alpha = 0.7) +  # Observed vs predicted points
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.2, alpha = 0.5) +  # Add CI bars
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +  # Diagonal line
  theme_minimal() +
  labs(
    x = "Observed Faith PD",
    y = "Predicted Faith PD (Posterior Mean ± 95% CI)"
  )


#ggsave(here::here("output/models/missing_data_div_nbinom_unconstr_2_trueage_islandindex_interaction_v2/p_pred_div.png"), height = 6, width = 9)

```

Strong Fit on Means:

Despite uncertainty, the model captures the expected patterns in Faith PD, supporting the relevance of predictors like habitat, brightness, and island characteristics.

Insights Into Broad Patterns:

The plot reinforces that the model is more reliable for understanding broad-scale trends than for precise predictions of Faith PD for individual species.

Improvement:

The observed discrepancies and large errors point to potential areas for improvement, such as:
Including additional ecological predictors.
Incorporating interactions among predictors.
Considering species-specific effects.
...
ggsave(here::here("output/figures/posterior_pars/figure_posteriors_load_diversity.png"),
       height = 7,
       width = 10)
```
```{r}
...
ggsave(here::here("output/figures/posterior_pars/figure_posteriors_brightness.png"),
       height = 7,
       width = 5)
```
...
# Model with mismatch instead of brightness
...
We will simply change the name of the dataset and variables (brightness for mismatch):

```{r}
# df_bright_noNA <- df_mismatch
# df_bright_noNA <- df_bright_noNA[!is.na(df_bright_noNA$mismatch), ]
...
```{r}
#| class-output: stan
load_nbinom_unconstr <- cmdstan_model(here::here("stan/load_nbinom_unconstr_2_trueage_islandindex_mismatch.stan"),
                                          pedantic = TRUE)
```

```{r}
# df_bright_noNA$mismatch_scaled <- scale(df_bright_noNA$mismatch)  # Normalize to mean 0, SD 1
```


```{r}
df_mismatch_clean <- df_mismatch %>%
...
init_fun <- function() {
  list(
    mu_mismatch = 0,
    sigma_mismatch = 1,
    alpha_mismatch = 0.5,  # Avoid extreme skewness issues
    mu_load = 0,
    phi_load = 1
  )
}
...
# Use island-level data in Stan
unconstr_mismatchload_samp <- load_nbinom_unconstr$sample(
  data = list(
  N_spp = nrow(sp_code_island_df),  # Number of species
  N_island = nrow(island_df),  # Number of unique islands
  sp_id = sp_code_island_df$species_code,
  island_area = island_df$island_area,  # Island-level area
  min_emergence = island_df$min_emergence,  # Island-level minimum emergence
  ...
  N_mismatch = nrow(df_mismatch_clean), 
  mismatch_obs = df_mismatch_clean$mismatch,
  sp_index_mismatch = df_mismatch_clean$species_code,
  
  # Nematode load data
  N_load = nrow(df_load),
  nematode_count = df_load$nematode_count,
  sp_index_load = df_load$species_code,
  island_index_load = df_load$new_island_index
),
  chains = 10, parallel_chains = 10, refresh = 0,
  iter_sampling = 500,
  output_dir = here::here("output/models/missing_data_load_mismatch_nbinom_unconstr_2_trueage_islandindex/model_output"),
init = init_fun
)
...
```{r}
# Define manual colors for islands
island_colors <- c(
  "Santa_Cruz" = "#66c2a5",
...
```{r}
# Compute mean age for each island and add it to sp_code_island_df
sp_code_island_df <- sp_code_island_df %>%
  mutate(mean_age = (min_emergence + max_emergence) / 2)

# Get the island order based on mean age (youngest to oldest)
island_order <- sp_code_island_df %>%
  group_by(island) %>%
  summarize(mean_age = mean(mean_age, na.rm = TRUE)) %>%
  arrange(mean_age) %>%
  pull(island)

# Add island factor with levels ordered by age
sp_code_island_df <- sp_code_island_df %>%
...
# Prepare the brightness data with mean age
brightness_data <- df_bright_noNA %>%
  left_join(sp_code_island_df %>% 
              select(island, mean_age, species_code, species_name), 
            by = "species_code") %>%
  mutate(island = factor(island, levels = names(island_colors)))  # Align factor levels with island colors

# Filter to include only islands with data
brightness_data <- brightness_data %>%
  filter(!is.na(mean_age))

# Create the brightness plot
brightness_plot <- ggplot(brightness_data, 
                          aes(x = reorder(species_name, mean_age), y = brightness, fill = island)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +  # Boxplot without outliers for clarity
  geom_jitter(color = "black", size = 0.5, alpha = 0.3, width = 0.2) +  # Raw data points in black
  scale_fill_manual(values = island_colors) +  # Manual colors for islands
  theme_minimal() +
  labs(x = "Species (ordered by Island Mean Age)", 
       y = "Brightness", 
       fill = "Island") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
# Prepare the habitat data with mean age
habitat_data <- df_hab %>%
  left_join(sp_code_island_df, by = "species_code") %>%
  pivot_longer(cols = c(arboreal, terrestrial), 
               names_to = "habitat_type", 
               values_to = "count") %>%
  mutate(island = factor(island, levels = island_order))  # Align factor levels with island order

# Filter to include only islands with data
habitat_data <- habitat_data %>%
  filter(!is.na(mean_age))

# Create the habitat plot
habitat_plot <- ggplot(habitat_data, 
                       aes(x = reorder(species_name, mean_age), y = count, fill = island, pattern = habitat_type)) +
  geom_bar_pattern(stat = "identity", position = position_dodge(), 
                   pattern_density = 0.1, pattern_fill = "black", pattern_spacing = 0.02) +
  scale_fill_manual(values = island_colors) +  # Manual colors for islands
  scale_pattern_manual(values = c("none", "stripe")) +  # Patterns for habitat types
  theme_minimal() +
  labs(x = "Species (ordered by Island Mean Age)", 
       y = "Habitat Counts", 
       fill = "Island", 
       pattern = "Habitat Type") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  theme(legend.position = "none")
...
```{r}
# Prepare the vegetation data with mean age
vegetation_data <- df_veg %>%
  left_join(sp_code_island_df, by = "species_code") %>%
  pivot_longer(cols = c(arid, humid), 
...
# Create the vegetation plot
vegetation_plot <- ggplot(vegetation_data, 
                          aes(x = reorder(species_name, mean_age), y = count, fill = island, pattern = vegetation_type)) +
  geom_bar_pattern(stat = "identity", position = position_dodge(), 
                   pattern_density = 0.1, pattern_fill = "black", pattern_spacing = 0.02) +
  scale_fill_manual(values = island_colors) +  # Manual colors for islands
  scale_pattern_manual(values = c("none", "stripe")) +  # Patterns for vegetation types
  theme_minimal() +
  labs(x = "Species (ordered by Island Mean Age)", 
       y = "Vegetation Counts", 
       fill = "Island", 
...
```
```{r}
# Prepare the nematode load data with mean age
nematode_data <- df_load %>%
  left_join(sp_code_island_df %>% 
              select(mean_age, species_code), 
            by = "species_code") %>%
  mutate(island = factor(island, levels = island_order))  # Align factor levels with island order

# Filter to include only islands with data
nematode_data <- nematode_data %>%
  filter(!is.na(mean_age))

# Create the nematode load plot
nematode_plot <- ggplot(nematode_data, 
                        aes(x = reorder(species_name, mean_age), y = nematode_count, fill = island)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +  # Boxplot without outliers for clarity
  geom_jitter(color = "black", size = 0.5, alpha = 0.3, width = 0.2) +  # Raw data points in black
  scale_fill_manual(values = island_colors) +  # Manual colors for islands
  theme_minimal() +
...
```{r}
# Prepare the diversity data with mean age
diversity_data <- faith_pd_table %>%
  left_join(sp_code_island_df %>% 
              select(mean_age, species_code), 
            by = "species_code") %>%
  mutate(island = factor(island, levels = island_order))  # Align factor levels with island order
...
# Create the diversity plot
diversity_plot <- ggplot(diversity_data, 
                         aes(x = reorder(species_name, mean_age), y = faith_pd, fill = island)) +
  geom_bar(stat = "identity", alpha = 0.7) +  # Barplot for diversity values
  scale_fill_manual(values = island_colors) +  # Manual colors for islands
  theme_minimal() +
  labs(x = "Species (ordered by Island Mean Age)", 
       y = "Diversity (Faith's PD)", 
       fill = "Island") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  theme(legend.position = "none")
...
# Read the shapefile
galapagos_map <- st_read(here::here("../maps/gal_island.shp"))

# Create a mapping between `isla` in shapefile and the dataset island names
island_name_mapping <- data.frame(
  isla = c("SantaCruz", "SanCristobal", "Santiago", "Isabela", "Fernandina",
           "Floreana", "Rabida", "SantaFe", "Espanola", "Pinzon",
           "Pinta", "Champion", "GardnerES"),  # Match names from galapagos_map
  dataset_name = names(island_colors)
)

# Merge the shapefile with island colors
galapagos_map <- galapagos_map %>%
  left_join(island_name_mapping, by = "isla") %>%
  mutate(fill_color = island_colors[dataset_name])  # Assign colors based on dataset names

# Create the map plot
galapagos_plot <- ggplot(galapagos_map) +
...
# Display the map
print(galapagos_plot)

ggsave(here::here("output/figures/map_colored.png"), height = 6, width = 6)
...
ggarrange(
  ...
  habitat_plot +
    theme(legend.position = "none", axis.title.x = element_blank(),
          plot.margin = margin(t = 9, r = 20, b = 5, l = 5)),
  vegetation_plot +
    theme(legend.position = "none", axis.title.x = element_blank(),
          plot.margin = margin(t = 9, r = 20, b = 5, l = 5)),
  nematode_plot +
    theme(legend.position = "none"),  # Keep y-axis label
  diversity_plot +
    theme(legend.position = "none"),  # Keep y-axis label
  nrow = 3,
  ncol = 2,
  labels = LETTERS[1:6],
  label.x = 0.05,  # Adjust horizontal position of labels
  label.y = 1.02,  # Adjust vertical position of labels
  hjust = -0.5,    # Adjust label alignment for spacing
  vjust = 1        # Adjust label alignment for spacing
)+
  theme(plot.margin = margin(t = 20, r = 10, b = 10, l = 10))  # Add top margin

#ggsave(here::here("output/figures/raw_data/raw_data_plot.png"), height = 15, width = 13)
```
