...
# Champion as Floreana

df_load_rename <- df_load_rename %>%
  mutate(island = ifelse(island == "Champion", "Floreana", island))
...
plot_brightness <- ggplot(brightness_data, aes(x = value, y = parameter, fill = type, alpha = type)) +
  geom_density_ridges(scale = 1.2, rel_min_height = 0.01) +
  scale_fill_manual(values = c("Posterior" = "grey20", "Prior" = "grey80")) +
  scale_alpha_manual(values = c("Posterior" = 1, "Prior" = 0.5)) +  # Less transparency for Prior
  theme_minimal() +
  labs(
...
plot_load <- ggplot(load_data, aes(x = value, y = parameter, fill = type, alpha = type)) +
  geom_density_ridges(scale = 1.2, rel_min_height = 0.01) +
  scale_fill_manual(values = c("Posterior" = "grey20", "Prior" = "grey80")) +
  scale_alpha_manual(values = c("Posterior" = 1, "Prior" = 0.5)) +  # Less transparency for Prior
  theme_minimal() +
...
```{r}
# Select only the parameters related to slopes
...
# Compute the mean of .value (rvar) before ordering
hab_rvars <- hab_rvars %>%
  mutate(mean_value = mean(.value)) %>%  # Extract mean from rvar
  mutate(species_name = fct_reorder(species_name, mean_value, .desc = TRUE))

# Create the plot
p_pred_hab <- ggplot(hab_rvars, aes(x = species_name, ydist = .value)) + 
  stat_pointinterval() + 
...
# Create the plot
p_pred_veg <- ggplot(veg_rvars, aes(x = species_name, ydist = .value)) + 
  stat_pointinterval() + 
  ylab("Arid probability")
...
```{r}
plots_load_model_predictions <- ggarrange(
  p_pred_hab,
  p_pred_veg,
  p_pred_bright,
  p_pred_load,
...
plots_load_model_predictions

ggsave(here::here("output/figures/model_checks/load_model_predictions.png"),
          height = 15,
          width = 10)
...
# Ridge plot for **effects on Brightness**
plot_brightness_div <- ggplot(brightness_data_div, aes(x = value, y = parameter, fill = type, alpha = type)) +
  geom_density_ridges(scale = 1.2, rel_min_height = 0.01) +
  scale_fill_manual(values = c("Posterior" = "grey20", "Prior" = "grey80")) +
  scale_alpha_manual(values = c("Posterior" = 1, "Prior" = 0.5)) +  # Less transparency for Prior
  theme_minimal() +
...
# Ridge plot for **effects on Load**
plot_div <- ggplot(diversity_data, aes(x = value, y = parameter, fill = type, alpha = type)) +
  geom_density_ridges(scale = 1.2, rel_min_height = 0.01) +
  scale_fill_manual(values = c("Posterior" = "grey20", "Prior" = "grey80")) +
  scale_alpha_manual(values = c("Posterior" = 1, "Prior" = 0.5)) +  # Less transparency for Prior
  theme_minimal() +
  labs(
    title = "Effects on nematode Faith PD",
    x = "Coefficient Estimate (??)",
    y = ""  # Simplified y-axis labels
  ) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
...
```{r}
# Gather predicted Faith's PD from model output
...
# Replace species codes with species names in div_rvars
div_rvars <- div_rvars %>%
  mutate(species_name = species_code_to_name[as.character(species_code)])

# Replace species codes with species names in faith_pd_table
faith_pd_table <- faith_pd_table %>%
  mutate(species_name = species_code_to_name[as.character(species_code)])

# Compute the mean observed Faith's PD for each species
df_div_mean <- faith_pd_table %>%
  group_by(species_name) %>%
  summarize(mean_faith_pd = mean(faith_pd, na.rm = TRUE))

# Reorder species names in div_rvars based on predicted diversity values
div_rvars <- div_rvars %>%
  mutate(species_name = forcats::fct_reorder(species_name, -.value))

# Plot predicted Faith's PD
p_pred_div <- div_rvars |> 
...
# Overlay observed Faith's PD on the plot (as jittered red points)
p_pred_div <- p_pred_div + 
  geom_point(aes(x = species_name, y = faith_pd), inherit.aes = FALSE, 
             data = faith_pd_table, 
             colour = "red3",
             size = 2.3, alpha = .6,
            # position = position_jitter(width = .3)
...
```{r}
plots_div_model_predictions <- ggarrange(
  p_pred_hab,
  p_pred_veg,
  p_pred_bright,
  p_pred_div,
...
plots_div_model_predictions

ggsave(here::here("output/figures/model_checks/div_model_predictions.png"),
          height = 15,
          width = 10)
...
```
```{r}
plot_brightness+theme(legend.position = "right")
