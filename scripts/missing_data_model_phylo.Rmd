...
```{r}
#| class-output: stan
all_spp_load <- cmdstan_model(here::here("stan/all_spp_load_nbinom.stan"),
                                          pedantic = TRUE)
...
# Data code including phylogenetic covariance matrix
all_spp_load_samp <- all_spp_load$sample(
  data = list(
  N_spp = nrow(sp_code_island_df),  # Number of species (47)
  N_island = max(sp_code_island_df$island_code),
  sp_id = sp_code_island_df$species_code,
...
iter_sampling = 500,
output_dir = here::here("output/models/missing_data_load_nbinom")
)
...
all_spp_load_samp <- as_cmdstan_fit(dir(here::here("output/models/missing_data_load_nbinom"), full.names = TRUE))
```
