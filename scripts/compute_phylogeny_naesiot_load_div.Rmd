...
plot_tree_load_fpd <- ggtree(tree) %<+% tree_plot_data +
  ...
  geom_segment(
    data = tree_plot_data %>% filter(faith_pd > 0),  # Only species with non-zero Faith PD
  ...
  geom_segment(
    data = tree_plot_data %>% filter(mean_load > 0),  # Only non-zero values
