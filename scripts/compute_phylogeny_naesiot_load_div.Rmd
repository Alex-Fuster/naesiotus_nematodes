...
missing_species <- data.frame(
  species = c("sculpturatus2", "alethorhytidus", "chatoblack", "cfwolfi", "wolfi2", 
              ...
              "hemaerodes", "sp nov Volcan Wolf 1", "sp nov Volcan Wolf 2", 
              "sp nov Volcan Wolf 3", "elaeodes", "simrothi2", "Brantsnail", 
              "spnovPinta1", "sp nov Pinta 2", "jacobi", "sp nov pinzon 1", 
...
tree_data <- data.frame(label = tree$tip.label) %>%
  left_join(species_names_df, by = c("label" = "species")) %>%
  left_join(mean_load_per_species, by = c("species_name" = "species_name")) %>%
  left_join(faith_pd_table |> select(species_name, faith_pd), by = c("species_name" = "species_name")) |> 
  mutate(
...
tree_data <- tree_data %>%
  left_join(tree_plot_data %>% select(label, y), by = "label")  # Add y positions to tree_data
...
tree_plot_data <- tree_plot_data %>%
  left_join(tree_data %>% select(label, mean_load, island, faith_pd), by = "label") %>%
  mutate(mean_load = ifelse(is.na(mean_load), 0, mean_load))  # Replace NA loads with 0


p <- ggtree(tree) %<+% tree_plot_data +
  geom_tree(aes(color = island), size = 0.8) +  # Color branches by island
  geom_tiplab(aes(label = label), hjust = -0.2, size = 2.5) +  # Add tip labels
...
p +
  ...
  geom_segment(
    data = tree_plot_data %>% filter(faith_pd > 0),  # Only species with non-zero mean load
    aes(x = max(node.depth.edgelength(tree)) * 1.5,  # Start of the bar
        xend = max(node.depth.edgelength(tree)) * 1.55,  # End of the bar
        y = y, yend = y, color = faith_pd),  # Map mean load to color gradient
