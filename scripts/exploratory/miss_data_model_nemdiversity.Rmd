...
Correct species min and max emergence
...
sp_code_island_df[which(sp_code_island_df$island_code == 9), "max_emergence"] <- 0.8
...
## Negative binomial model constrained priors (+habitat & veg models + measurement error on island age)
...
```{r}
#| class-output: stan
model_div_constr <- cmdstan_model(here::here("stan/div_nbinom_constr_2_trueage.stan"),
                                          pedantic = TRUE)
```
...
constr_div_samp <- model_div_constr$sample(
  ...
  chains = 10, parallel_chains = 10, refresh = 0,
iter_sampling = 500,
output_dir = here::here("output/models/div_nbinom_constr_2_trueage/model_output")
)
...
ggsave(here::here("output/models/div_nbinom_constr_2_trueage/priors_posteriors.png"), height = 10, width = 8)
```