...
slope_age_bright ~ normal(0, 0.5) T[, 0];  // Biased towards negative (Kraemer et al. 2019)


```{r}
#| class-output: stan
all_spp_load_phylo_constrained <- cmdstan_model(here::here("stan/all_spp_load_phylo_constrained.stan"),
                                          pedantic = TRUE)

all_spp_load_phylo_constrained
```
...
# Data code including phylogenetic covariance matrix
all_spp_load_constrained_samp <- all_spp_load_phylo_constrained$sample(
  data = list(
  N_spp = nrow(sp_code_island_df),  # Number of species (47)
  N_island = max(sp_code_island_df$island_code),
  sp_id = sp_code_island_df$species_code,
  ...
  chains = 4, parallel_chains = 4, refresh = 0,
output_dir = here::here("output/models/missing_data_load_phylo_constrained")
)
...
```{r}
# read in the model from above
all_spp_load_constrained_samp <- as_cmdstan_fit(dir(here::here("output/models/missing_data_load_phylo_constrained"), full.names = TRUE))
```
...
```{r}
# Extract the posterior samples
posterior_samples <- all_spp_load_constrained_samp$draws()
