...
data <- read_excel(here::here("data/19july24_datalab.xlsx"), sheet = "pools")
...
```{r}
# Filter for positive pools only
positive_pools <- data %>%
  filter(id_positive_pull > 0)

# Summarize the number of individuals per species code across all positive pulls
summary_data <- positive_pools %>%
  group_by(code) %>%
  summarise(count = n(), .groups = 'drop')

# Plot using ggplot
ggplot(summary_data, aes(x = code, y = count)) +
  geom_point(size = 2, color = "black", shape = 4) +
  labs(
       x = "Species Code",
       y = "N snail individuals in positive pools") +
  theme_classic()

#ggsave(here::here("output/figures/nsnails_positivepools.png"), height = 5, width = 7)
```
...
#ggsave(here::here("output/figures/nsnails_positivepools.png"), height = 5, width = 9)
