...
# Adjust the min and max emergence values where max_emergence is NA
sp_code_island_df <- sp_code_island_df |>
  dplyr::mutate(
    max_emergence = ifelse(is.na(max_emergence)| max_emergence == min_emergence, min_emergence + small_amount, max_emergence))
...
## Negative binomial model non-informed priors neither brightness nor load  (+hab/veg models and measurement error in island age)


```{r}
#| class-output: stan
load_nbinom_unconstr <- cmdstan_model(here::here("stan/load_nbinom_unconstr_2_trueage.stan"),
                                          pedantic = TRUE)
```
...
unconstr_brightload_samp <- load_nbinom_unconstr$sample(
  ...
  min_emergence = as.numeric(sp_code_island_df$min_emergence),
  max_emergence = as.numeric(sp_code_island_df$max_emergence),
...
iter_sampling = 500,
output_dir = here::here("output/models/missing_data_load_nbinom_unconstr_2_trueage/model_output")
)
...
unconstr_brightload_samp <- as_cmdstan_fit(dir(here::here("output/models/missing_data_load_nbinom_unconstr_2_trueage/model_output"), full.names = TRUE))
```
...
#ggsave(here::here("output/models/missing_data_load_nbinom_unconstr_2_trueage/priors_posteriors.png"), height = 10, width = 8)
```
...
## Negative binomial model constrained priors for brightness (+hab/veg models and measurement error in island age)


```{r}
#| class-output: stan
load_nbinom_constr_2 <- cmdstan_model(here::here("stan/load_nbinom_constr_2_trueage.stan"),
                                          pedantic = TRUE)
```
...
# Data code including phylogenetic covariance matrix
constr_brightload_samp <- load_nbinom_constr_2$sample(
  data = list(
  N_spp = nrow(sp_code_island_df),  # Number of species (47)
  N_island = max(sp_code_island_df$island_code),
  sp_id = sp_code_island_df$species_code,
  island_area = sp_code_island_df$island_area,
  #island_age = sp_code_island_df$island_age,
  min_emergence = sp_code_island_df$min_emergence,
  max_emergence = sp_code_island_df$max_emergence,
  island_index_spp = sp_code_island_df$island_code,
  ...
  chains = 10, parallel_chains = 10, refresh = 0,
iter_sampling = 500,
output_dir = here::here("output/models/missing_data_load_nbinom_constr_2_trueage/model_output")
)
...
```{r}
# read in the model from above
constr_brightload_samp <- as_cmdstan_fit(dir(here::here("output/models/missing_data_load_nbinom_constr_2_trueage/model_output"), full.names = TRUE))
```
...
ggsave(here::here("output/models/missing_data_load_nbinom_constr_2_trueage/priors_posteriors.png"), height = 10, width = 8)
```