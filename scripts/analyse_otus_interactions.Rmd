...
## CENTRALITY IN THE REGIONAL NETWORK
...
#saveRDS(centralities_df, here::here("output/tables/centralities_regional_df.rds"))
```
...
for (island in valid_islands) {
  print(island)  # For debugging purposes
  ...
  island_snails <- island_metadata$naesiotus_sp
  
  # Subset bipartite matrix for snails from the current island
  island_matrix <- bipartite_matrix %>%
    filter(naesiotus_sp %in% island_snails) %>%
    column_to_rownames(var = "naesiotus_sp") %>%  # Use species names as row names
    as.matrix()  # Convert to matrix
  
  # Check if the matrix is valid (non-zero rows and columns)
  if (all(rowSums(island_matrix) == 0) || all(colSums(island_matrix) == 0)) {
  ...
  degree_centrality <- degree(g_island, v = snail_nodes)
  closeness_centrality <- closeness(g_island, v = snail_nodes)
  ...
  centralities_df <- centralities_df %>%
    mutate(across(degree:katz, ~ ifelse(is.na(.), 0, .)))
  
  # Standardize centralities (z-scores)
  centralities_df <- centralities_df %>%
    mutate(across(degree:katz, ~ scale(.) %>% as.numeric()))
  
  # Perform PCA on standardized centrality measures
  if (ncol(centralities_df[, -1]) > 1) {
    pca_result <- PCA(centralities_df[, -1], scale.unit = TRUE, graph = FALSE)
    pca_centrality <- as.data.frame(pca_result$ind$coord)[, 1]  # First principal component
  } else {
...
final_centralities_df <- bind_rows(island_centralities)

# View the final dataframe
head(final_centralities_df)

#saveRDS(final_centralities_df, here::here("output/tables/local_centralities_df.rds"))
